...
ConfigOption = collections.dtuple("ConfigOption", ("", "default", "help"))
CONFIG_OPTIONS = (
...
DEFAULT_CONFIG = {option.: option.default for option in CONFIG_OPTIONS}
...
class Datasette:
    def __init__(
        ...
        for file in self.files:
            ...
            if db. in self.databases:
                raise Exception("Multiple files with same stem: {}".format(db.))
            self.databases[db.] = db
        ...
        if self.plugins_dir:
            for file in os.listdir(self.plugins_dir):
                ...
                mod = module_from_path(filepath, =file)
                try:
                    pm.register(mod)
                except ValueError:
    ...
    async def run_sanity_checks(self):
        ...
        for database_, database in self.databases.items():
            # Run pragma_info on every table
            for table in await database.table_s():
                try:
                    await self.execute(
                        database_,
    ...
    def config_dict(self):
        ...
        return {option.: self.config(option.) for option in CONFIG_OPTIONS}
    ...
    def plugin_config(self, plugin_, database=None, table=None, fallback=True):
        ...
        if plugins is None:
            return None
        plugin_config = plugins.get(plugin_)
    ...
    def get_canned_queries(self, database_):
        queries = self.metadata("queries", database=database_, fallback=False) or {}
        s = queries.keys()
        return [self.get_canned_query(database_, ) for  in s]

    def get_canned_query(self, database_, query_):
        queries = self.metadata("queries", database=database_, fallback=False) or {}
        query = queries.get(query_)
        if query:
    ...
    def prepare_connection(self, conn):
        ...
        for , num_args, func in self.sqlite_functions:
            conn.create_function(, num_args, func)
    ...
    def connected_databases(self):
        return [
            ...
            for d in sorted(self.databases.values(), key=lambda d: d.)
        ]
    ...
    def plugins(self, show_all=False):
        ...
        if not show_all:
            ps = [p for p in ps if p[""] not in DEFAULT_PLUGINS]
        return [
            {
                "": p[""],
    ...
    async def execute_against_connection_in_thread(self, db_, fn):
        def in_thread():
            conn = getattr(connections, db_, None)
            if not conn:
                db = self.databases[db_]
                if db.is_memory:
                    conn = sqlite3.connect(":memory:")
                else:
    ...
    async def execute(
        ...
        """Executes sql against db_ in a thread"""
        ...
        with trace("sql", database=db_, sql=sql.strip(), params=params):
            results = await self.execute_against_connection_in_thread(
                db_, sql_operation_in_thread
            )
        return results
    ...
    def app(self):
        ...
        for path, dir in self.static_mounts:
            add_route(asgi_static(dir), r"/" + path + "/(?P<path>.*)$")

        # Mount any plugin static/ directories
        for plugin in get_plugins(pm):
            if plugin["static_path"]:
                modpath = "/-/static-plugins/{}/(?P<path>.*)$".format(plugin[""])
        ...
        add_route(
            DatabaseDownload.as_asgi(self), r"/(?P<db_>[^/]+?)(?P<as_db>\.db)$"
        )
        add_route(
        ...
        add_route(
            ...
            r"/(?P<db_>[^/]+)/(?P<table_and_format>[^/]+?$)",
        )
        add_route(
            RowView.as_asgi(self),
            r"/(?P<db_>[^/]+)/(?P<table>[^/]+?)/(?P<pk_path>[^/]+?)(?P<as_format>"
            + renderer_regex
            + r")?$",
        )
        ...
        async def setup_db():
            ...
            for db, database in self.databases.items():
